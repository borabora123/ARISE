...
```bash
set PYTHONPATH=${PYTHONPATH:/path/to/arise-2025}
```
...
```bash
icacls preprocess_for_yolo.sh
./preprocess_for_yolo.sh --csv /dataset/bboxes.csv --img_dir /dataset/scores.csv --split_info_path /dataset/train_val_split.json
```
...
```bash
icacls preprocess_for_classifier.sh
